...
This script is the previous test client updated for use with python 3
...
def create_callback_server():
    class CallbackHandler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(
                b"OAuth request token fetched and authorized;"
...
def get_api_resource(session):
    resource_url = (
        input("Resource relative url (e.g. %s): " % DEFAULT_API_RESOURCE)
        or DEFAULT_API_RESOURCE
    )
...
def run_tests():
    ...
    # use default values.
    CONSUMER_KEY = input("consumer key: ") or CONSUMER_KEY
    CONSUMER_SECRET = input("consumer secret: ") or CONSUMER_SECRET
    SERVER_URL = input("server base url: ") or SERVER_URL
